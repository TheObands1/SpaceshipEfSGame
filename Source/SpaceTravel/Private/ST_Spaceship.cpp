...
AST_Spaceship::AST_Spaceship()
{
	...
	bIsThisTheFirstTakeOff = true;
	bDidShipLandCorrectly = false;
	bDidShipLand = false;
...
{
	if (!bDidShipLand && CurrentFuelAmount != 0)
	{
...
void AST_Spaceship::Land()
{
	...
	FRotator RotationToSimulatePerfectLanding = UKismetMathLibrary::MakeRotFromZY(UKismetMathLibrary::GetForwardVector(RotationToPointAtPlanet), SpaceshipMeshComponent->GetRightVector());


	SetActorRotation(FRotator(RotationToSimulatePerfectLanding.Roll , 0, 0));
	*/
	...
	{
		//RotationToSimulatePerfectLanding's Roll must always be negative

		if (RotationToSimulatePerfectLanding.Roll < 0)
		{
			SetActorRotation(FRotator(RotationToSimulatePerfectLanding.Roll, 0, 0));
		}
		else
		{
			SetActorRotation(FRotator(RotationToSimulatePerfectLanding.Roll*-1, 0, 0));
		}

	}
	else //The spaceship is at the Right side of the planet
	{
		// RotationToSimulatePerfectLanding's Roll must always be positive
		SetActorRotation(FRotator(UKismetMathLibrary::Abs(RotationToSimulatePerfectLanding.Roll), 0, 0));
	}
	*/
